...
pub async fn login_google(
    ...
    let user = if let Some(user) = existing_user {
        ...
        // User does not exist - JIT Provisioning
        let user_id = Uuid::new_v4().to_string();
        let now = Utc::now().timestamp();
        let sanitized_name = sanitize_string(&name);

        // Sentinel Hash for Google Users
        let sentinel_hash = "GOOGLE_OAUTH_USER";
        ...
        // Send welcome email logic copy/paste or refactor (omitted for brevity, can assume JIT doesn't need implicit welcome or add todo)
        // For now, let's skip welcome email to keep handler concise, or spawn strictly if needed.

        User {
            id: user_id,
            email,
            password_hash: sentinel_hash.to_string(),
            name: sanitized_name,
