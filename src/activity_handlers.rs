...
pub async fn send_whatsapp_reminder(
    ...
    match crate::whatsapp_service::send_reminder_whatsapp(
        &req.phone,
...
pub async fn send_email_reminder(
    ...
    // 3. Send email
    if let Err(e) =
        crate::email_service::send_reminder_email(&email, &poll_title, &req.message).await
    {
    ...
    crate::activity_handlers::log_activity(
        ...
        "reminder_sent",
        req.user_id,
        "System".to_string(), // TODO: Better user name if possible
        Some(req.session_id),
        Some(poll_title),
    )
    .await
